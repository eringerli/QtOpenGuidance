...
void PoseSynchroniser::setPosition( const Eigen::Vector3d& position ) {
...
void PoseSynchroniser::setOrientation( const Eigen::Quaterniond& value ) {
  orientation = value;
}

void PoseSynchroniser::emitConfigSignals() {
  Q_EMIT poseChanged( position, orientation, PoseOption::NoOptions );
}

QNEBlock* PoseSynchroniserFactory::createBlock( QGraphicsScene* scene, int id ) {
