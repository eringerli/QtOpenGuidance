...
ValueTransmissionState::ValueTransmissionState( int id ) : ValueTransmissionBase( id ) {
  reader = std::make_unique<QCborStreamReader>();
}

void ValueTransmissionState::setState( const bool state ) {
  QCborMap map;
...
void ValueTransmissionState::dataReceive( const QByteArray& data ) {
  reader->addData( data );
...
QNEBlock* ValueTransmissionStateFactory::createBlock( QGraphicsScene* scene, int id ) {
