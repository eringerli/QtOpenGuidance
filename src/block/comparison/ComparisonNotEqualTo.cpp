...
void ComparisonNotEqualTo::emitConfigSignals() {
  Q_EMIT stateChanged( result );
}

void ComparisonNotEqualTo::setValueA( double number ) {
  numberA = number;
  operation();
}

void ComparisonNotEqualTo::setValueB( double number ) {
  numberB = number;
  operation();
}

void ComparisonNotEqualTo::operation() {
  result = !qFuzzyCompare( numberA, numberB );
  Q_EMIT stateChanged( result );
}

QNEBlock* ComparisonNotEqualToFactory::createBlock( QGraphicsScene* scene, int id ) {
