...
void ComparisonLessThan::emitConfigSignals() {
  Q_EMIT stateChanged( result );
}

void ComparisonLessThan::setValueA( double number ) {
  numberA = number;
  operation();
}

void ComparisonLessThan::setValueB( double number ) {
  numberB = number;
  operation();
}

void ComparisonLessThan::operation() {
  result = numberA < numberB;
  Q_EMIT stateChanged( result );
}

QNEBlock* ComparisonLessThanFactory::createBlock( QGraphicsScene* scene, int id ) {
