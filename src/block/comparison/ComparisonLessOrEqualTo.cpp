...
void ComparisonLessOrEqualTo::emitConfigSignals() {
  Q_EMIT stateChanged( result );
}

void ComparisonLessOrEqualTo::setValueA( double number ) {
  numberA = number;
  operation();
}

void ComparisonLessOrEqualTo::setValueB( double number ) {
  numberB = number;
  operation();
}

void ComparisonLessOrEqualTo::operation() {
  result = numberA <= numberB;
  Q_EMIT stateChanged( result );
}

QNEBlock* ComparisonLessOrEqualToFactory::createBlock( QGraphicsScene* scene, int id ) {
