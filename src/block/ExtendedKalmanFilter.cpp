...
void ExtendedKalmanFilter::setPosition( const Eigen::Vector3d& position ) {
  predict();
...
void ExtendedKalmanFilter::setVelocity3D( const Eigen::Vector3d& velocity3D ) {
  this->velocity3D = velocity3D;
}

void ExtendedKalmanFilter::setImuData( const Eigen::Quaterniond& orientation, const Eigen::Vector3d& accelerometerData, const Eigen::Vector3d& gyroData ) {
  predict();
...
void ExtendedKalmanFilter::setOrientation( const Eigen::Quaterniond& value ) {
  orientation = value;
}

void ExtendedKalmanFilter::setOrientationCorrection( const Eigen::Quaterniond& value ) {
...
void ExtendedKalmanFilter::emitPoseFromEKF() {
  auto x = ekf.getState();
...
void ExtendedKalmanFilter::emitConfigSignals() {
  Q_EMIT poseChanged( position, orientation, PoseOption::NoOptions );
}

QNEBlock* ExtendedKalmanFilterFactory::createBlock( QGraphicsScene* scene, int id ) {
