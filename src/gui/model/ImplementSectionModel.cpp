...
int
ImplementSectionModel::rowCount( const QModelIndex& ) const {
...
int
ImplementSectionModel::columnCount( const QModelIndex& ) const {
  return 3;
}
    ...
    if( size_t( index.row() ) < ( implement->sections.size() - 1 ) ) {
      switch( index.column() ) {
        case 0:
          ...
          Q_EMIT implement->emitConfigSignals();
...
void
ImplementSectionModel::setDatasource( Implement* implement ) {
  beginResetModel();
