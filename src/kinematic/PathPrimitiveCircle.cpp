...
#include "PathPrimitiveCircle.h"
#include "helpers/cgalHelper.h"

inline void
PathPrimitiveCircle::setHelpers() {
  {
...
PathPrimitiveCircle::PathPrimitiveCircle( const Point_2& startPoint,
...
bool
PathPrimitiveCircle::operator==( PathPrimitiveCircle& b ) const {
  return startPoint == b.startPoint && endPoint == b.endPoint && centerPoint == b.centerPoint;
}

bool
PathPrimitiveCircle::operator==( const PathPrimitiveCircle& b ) const {
  return startPoint == b.startPoint && endPoint == b.endPoint && centerPoint == b.centerPoint;
}

std::shared_ptr< PathPrimitive >
PathPrimitiveCircle::createReverse() {
  return std::make_shared< PathPrimitiveCircle >( endPoint, middlePoint, startPoint, implementWidth, anyDirection, passNumber );
}

std::shared_ptr< PathPrimitive >
PathPrimitiveCircle::createNextPrimitive( bool left ) {
  auto offsetVector = polarOffsetRad( angleOfLineRadians( bisectorLine ) + M_PI, left ? implementWidth : -implementWidth );

  return std::make_shared< PathPrimitiveCircle >( startPoint - offsetVector,
...
void
PathPrimitiveCircle::print() {
  std::cout << "PathPrimitiveCircle: start: " << startPoint << ", middle: " << middlePoint << ", end: " << endPoint
            << ", center: " << centerPoint << ", radius: " << radius << ", curvature: " << curvatureValue << ", clockwise: " << arcClockWise
            << std::endl;
}

double
PathPrimitiveCircle::distanceToPointSquared( const Point_2 point ) {
  return std::pow( std::sqrt( CGAL::squared_distance( point, centerPoint ) ) - radius, 2 );
}

bool
PathPrimitiveCircle::isOn( const Point_2 point ) {
...
double
PathPrimitiveCircle::curvature() {
  return curvatureValue;
}

bool
PathPrimitiveCircle::intersectWithLine( const Line_2& lineToIntersect, Point_2& resultingPoint ) {
  EpickCircularConverter epickToCircular;
...
Line_2
PathPrimitiveCircle::perpendicularAtPoint( const Point_2 point ) {
  return Line_2( centerPoint, point );
}

Point_2
PathPrimitiveCircle::orthogonalProjection( const Point_2 point ) {
...
Line_2&
PathPrimitiveCircle::supportingLine( const Point_2 point ) {
  Point_2 resultingPoint;
...
void
PathPrimitiveCircle::transform( const Aff_transformation_2& transformation ) {
