...
FixedKinematicPrimitive::setPose( const Eigen::Vector3d&     position,
                                  ...
                                  const PoseOption::Options& options ) {
  ...
  positionCalculated =
    Eigen::Vector3d( position.x() + positionCorrection.x(), position.y() + positionCorrection.y(), position.z() + positionCorrection.z() );
...
FixedKinematicPrimitiveFactory::createBlock( QGraphicsScene* scene, int id ) {
  ...
  b->addInputPort( QStringLiteral( "Pose In" ),
                   QLatin1String( SLOT( setPose( const Eigen::Vector3d&, const Eigen::Quaterniond&, const PoseOption::Options& ) ) ) );

  b->addOutputPort(
    QStringLiteral( "Pose Out" ),
    QLatin1String( SIGNAL( poseChanged( const Eigen::Vector3d&, const Eigen::Quaterniond&, const PoseOption::Options& ) ) ) );
